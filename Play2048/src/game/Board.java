...
public class Board {
    ...
    public void addRandomTile() {
        ...
        if (!openSpaces.isEmpty()) {
            ...
            BoardSpot Rspot = openSpaces.get(t);

            int i = Rspot.getRow();
            int j = Rspot.getCol();

            double Rvalue = StdRandom.uniform();
    ...
    public void swipeLeft() {
        ...
        for (int i = 0; i < gameBoard.length; i++) {
            ...
            for (int j = 0; j < gameBoard[i].length; j++) {
                if (gameBoard[i][j] != 0) {
                    if (j != zIndex) {
                        gameBoard[i][zIndex] = gameBoard[i][j];
                        gameBoard[i][j] = 0;
                    }
                    zIndex++;
                }
    ...
    public void transpose() {
        ...
        int[][] transposed = new int[gameBoard.length][gameBoard.length];
    ...
    public void flipRows() {
        ...
        for (int i = 0; i < gameBoard.length; i++) {
            int left = 0;
            int right = gameBoard[i].length - 1;
            while (left < right) {
